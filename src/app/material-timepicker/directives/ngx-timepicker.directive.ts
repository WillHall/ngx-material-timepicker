...
export class TimepickerDirective implements ControlValueAccessor, OnDestroy, OnChanges {
  ...
  set value(value: string) {
    ...
    this._value = formatTime(value, this._format, this.enableSeconds);
  ...
  private set defaultTime(time: string) {
    if (this.isValueAvailableToUpdate()) {
      this._timepicker.setDefaultTime({
        time: formatTime(time, this._format, this.enableSeconds),
  ...
  private isValueAvailableToUpdate(): boolean {
    ...
    const isBetween = (this._min && this._max) && convertTimeToMoment(this._value).isBetween(this._min, this._max, 'minutes');
...
function formatTime(time: string, format = 12, enableSeconds?: boolean): string {
  const timeFormat = format === 24 ? TimeFormat.TWENTY_FOUR : TimeFormat.TWELVE;
  const fullFormat = enableSeconds ? timeFormat.replace('mm', 'mm:ss') : timeFormat;
  return moment(time, fullFormat).format(fullFormat);
}

function convertTimeToMoment(time: string, enableSeconds?: boolean): Moment {
  const format = enableSeconds ? TimeFormat.TWELVE.replace('mm', 'mm:ss') : TimeFormat.TWELVE;
