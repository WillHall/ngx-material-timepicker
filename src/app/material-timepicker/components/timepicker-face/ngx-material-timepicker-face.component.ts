...
export class NgxMaterialTimepickerFaceComponent implements AfterViewInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if ((changes['faceTime'] && changes['faceTime'].currentValue) && (changes['selectedTime'] && changes['selectedTime'].currentValue)) {
  ...
  selectTime(e: MouseEvent | Touch): void {
    ...
    const angleStep = [TimeUnit.MINUTE, TimeUnit.SECOND].indexOf(this.unit) !== -1  ? 6 : 30;
    const roundedAngle = isInnerClockChosen ? roundAngle(circleAngle, angleStep) + 360 : roundAngle(circleAngle, angleStep);
